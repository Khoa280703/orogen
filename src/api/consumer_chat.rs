...
use crate::api::consumer_api_support::{
    build_user_usage_context, mark_account_success, start_chat_stream_with_retry,
};
...
pub async fn create_conversation(
    ...
    let conversation =
        conversations::create_conversation(&state.db, user.user_id, body.title.as_deref(), Some(&model))
            .await
            .map_err(db_error)?;
...
pub async fn get_conversation(
    ...
    let items = messages::list_messages(&state.db, id).await.map_err(db_error)?;
    Ok(Json(ConversationResponse { conversation, messages: items }))
}
...
pub async fn send_message(
    ...
    let resolved_model = if let Some(model) = select_active_chat_model(&state.db, &requested_model).await {
        model
    } else if let Some(model) = select_active_chat_model(&state.db, DEFAULT_CHAT_MODEL).await {
        model
    } else {
        return Err(AppError::Internal("No active chat model is available".into()));
    };
    ...
    if messages::count_messages(&state.db, id).await.map_err(db_error)? == 0 {
    ...
    let history = messages::list_messages(&state.db, id)
        ...
        .map_err(db_error)?
        .into_iter()
        .map(|item| ChatMessage { role: item.role, content: item.content })
        .collect::<Vec<_>>();

    let provider = state
        .providers
        .chat_provider(&provider_slug)
        .ok_or_else(|| AppError::Internal(format!("Chat provider not registered: {provider_slug}")))?;
    let usage = {
        let mut usage = build_user_usage_context(user.user_id, model.clone(), REQUEST_KIND_CHAT);
        usage.plan_id = Some(plan_id);
        usage
    };
    let (mut rx, account_id, _account_name) =
        start_chat_stream_with_retry(&state, provider, &usage.model, &history, "", &usage).await?;
...
async fn finish_chat(
    ...
    if success {
        ...
        let _ = state.pool.mark_failure().await;
    }
    record_usage(state, usage, account_id, status, duration_to_latency_ms(started.elapsed())).await;
}

fn default_limit() -> i64 { 20 }
fn db_error(error: sqlx::Error) -> AppError { AppError::Internal(format!("Database error: {error}")) }
