...
use crate::AppState;
use crate::api::consumer_api_support::{mark_account_success, start_chat_stream_with_retry};
...
pub async fn chat_completions(
    ...
    let usage_context =
        resolve_usage_context(&state, &api_key, &resolved_model.slug, REQUEST_KIND_CHAT).await?;
    ...
    let provider = state
        .providers
        .get_chat_provider(&resolved_model.provider_slug)
        .ok_or_else(|| {
    ...
    let (system_prompt, messages) = split_messages(&body.messages)?;
    ...
    let content = collect_completion(
        &state,
        provider,
        &resolved_model.slug,
        &system_prompt,
...
async fn stream_response(
    state: AppState,
    provider: std::sync::Arc<dyn crate::providers::chat_provider::ChatProvider>,
    model: String,
    ...
    let stream = stream! {
        match start_chat_stream_with_retry(
            &state,
            provider,
            &model,
            &messages,
        ...
        .await {
            Ok((mut rx, account_id, _account_name)) => {
                ...
                while let Some(event) = rx.recv().await {
                    match event {
                        ...
                        ChatStreamEvent::Error(error) => {
                            finish_completion(&state, &usage_context, account_id, "error", false, started_at).await;
...
async fn collect_completion(
    ...
    while let Some(event) = rx.recv().await {
        match event {
            ChatStreamEvent::Token(token) => content.push_str(&token),
            ChatStreamEvent::Thinking(_) => {}
            ChatStreamEvent::Done => {
                finish_completion(state, usage_context, account_id, "success", true, started_at)
                    .await;
                return Ok(content);
            }
            ChatStreamEvent::Error(error) => {
                finish_completion(state, usage_context, account_id, "error", false, started_at)
                    .await;
                return Err(AppError::GrokApi(error));
            }
        }
    }
    ...
    .await;
    Err(AppError::GrokApi("Upstream stream ended unexpectedly".into()))
}
...
mod tests {
    ...
    fn rejects_legacy_tool_call_payloads() {
        ...
        let error = super::split_messages(&parsed.messages).unwrap_err();
        assert!(error.to_string().contains("tool and function calling payloads"));
    }
}
