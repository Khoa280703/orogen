...
- Current snapshot: the vertical slice is done, the expanded suite now has manifest + bucket files + `40` English-first cases + `case-level dimension_checks`, and the report includes both `dimension_scores` and `dimension_proxies`; after the latest final `plan-repair-001` verifier-phrasing refinement the full phase is still not complete.
...
- That final refinement intentionally preferred stricter verifier semantics over an inflated pass rate or cleaner-looking heuristic verifier score, even though it kept the same `35/40` overall result.
...
- Current baseline snapshot:
  ...
  2. live eval: `35/40 = 0.875`
  3. report timestamp: `2026-04-15T07:33:06.280203+00:00`
  4. buckets:
     - `coding-fix`: `8/8`
  ...
  7. earlier false-red cleanup was only partially retained; the final `plan-repair-001` refinement kept the same `35/40` score but moved `plan-repair` toward stricter verifier semantics and explicitly preferred less false-green over preserving a prettier pass rate
  8. the newest snapshot is stricter about counting verifier-style post-remediation steps, so shallow redeploy-only or pre-remediation verification phrasing is less likely to over-score
  9. the suite is materially stronger as a screening baseline now, but some residual false-green and text-only heuristic scoring still remain
...
- Do not move into major pretraining or SFT work until the eval suite is strong enough to measure real differences.
